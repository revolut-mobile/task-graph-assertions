...
class WhenRequestedTest {
    ...
    @Test
    fun `task not requested - assertions skipped`(@TempDir projectDir: File) {
        createProject(
            projectDir, """
            |register(":check") {
            |    assertTasksNotTriggered.addAll(":build")
            |}
            |""".trimMargin()
        )
        val result = assertBuildSuccess(projectDir, ":build")
        Truth.assertThat(result.output).contains("Task ':check' was not requested. Skip assertions")
    }
